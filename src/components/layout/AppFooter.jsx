  ...
  return (
    <View className="bg-sunken-strong px-6 pb-10 pt-10">
      <View className="gap-6 border-b border-black/10 pb-8">
        {LINK_GROUPS.map((group, index) => (
          <View key={index} className="gap-2">
            {group.map((label) => (
              <Text key={label} className="text-xs font-light text-content">
                {label}
              </Text>
            ))}
          </View>
      ...
      <View className="pt-8">
        ...
        <Text className="mt-4 text-xs text-content-2">
          Matchroom Multi Sport Ltd, Mascalls, Mascalls Lane, Brentwood, Essex,
          England CM14 5LJ
        </Text>
        <Text className="mt-2 text-xs font-light text-content-2">
          © 2024 Matchroom Multi Sport Ltd. Bảo lưu mọi quyền.
        </Text>

        <View className="mt-6 flex-row items-center gap-5">
          <Text className="text-2xl font-black italic text-content">
            CAPS.tv
          </Text>
          <Facebook size={20} color={colors.content2} />
          <Instagram size={20} color={colors.content2} />
          <Youtube size={22} color={colors.content2} />
        </View>
      </View>
    </View>
  );
